...
export default function ATSChecker({ onSidebarToggle }) {
  ...
  useEffect(() => {
    ...
    const savedA = sessionStorage.getItem("ats_analysis_result");
    if (savedA) setAnalysisResult(JSON.parse(savedA));
  }, [previewUrl]);
          ...
          {!analysisResult && !isLoading && (
            <div className="bg-white border border-dashed border-zinc-200 rounded-2xl p-6 flex flex-col items-center text-center gap-3">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed max-w-[200px]">
                Upload your resume to see your ATS score and section breakdown
              </p>
            </div>
          )}
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-6 px-8 text-center">
                        ...
                        className={`
                          ...
                          ${
                            isDragging
                              ...
                              : "bg-white text-zinc-400 group-hover:text-zinc-500 shadow-zinc-200/80"
                          }
                        `}
                      >
                      ...
                      <div className="space-y-2">
                        <motion.p
                          ...
                          className={`text-xl font-semibold tracking-tight transition-colors duration-200
                            ${isDragging ? "text-indigo-600" : "text-zinc-700"}`}
                        >
                          {isDragging
                            ? "Release to upload"
                      ...
                      <motion.div
                        ...
                        transition={{ duration: 0.2 }}
                        className="flex items-start gap-2.5 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 max-w-xs text-left"
                      >
                        <AlertCircle
                          size={13}
                        ...
                        <p className="text-xs text-amber-700 leading-relaxed">
                          Use a{" "}
                          <span className="font-semibold">text-based PDF</span>{" "}
                          for best ATS results — scanned images won't be parsed.
                        </p>
                      </motion.div>
                    </div>
          ...
          <tbody>
            {errors.map((e, i) => (
              <tr
                key={i}
                onClick={() => onSelect(e)}
                className={`cursor-pointer border-b border-black/[0.04] transition-colors last:border-0
                  ${activeError === e ? v.active : v.hover}`}
              >
